...
_INJECTION_PATTERNS: tuple[re.Pattern[str], ...] = (
    ...
    re.compile(r"disregard\s+(all\s+)?(previous|prior|your)\s+(instructions?|rules?)", re.I),
    re.compile(r"reveal\s+(your\s+)?(system\s+)?(prompt|instructions?)", re.I),
    re.compile(r"(show|print|repeat|output)\s+(the\s+)?(system\s+)?(prompt|instructions?)", re.I),
